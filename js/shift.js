...
function updateCircle(i) {
  ...
  drawCircle(x, y, life, ttl, radius, hue, i);
...
function drawCircle(x, y, life, ttl, radius, hue, index) {
  ctx.a.save();
  ctx.a.fillStyle = `hsla(${hue},90%,30%,${fadeInOut(life, ttl)})`;

  // console.log("Index", index / 8, (index / 8) % 2 == 0);

  //if index is even, fillStyle is red
  if ((index / 8) % 3 == 0) {
    ctx.a.fillStyle = `hsla(${hue},90%,30%,${fadeInOut(life, ttl)})`;
    console.log(index / 8);
  } else {
    ctx.a.fillStyle = `hsla(${hue},0%,0%,${fadeInOut(life, ttl)})`;
  }
